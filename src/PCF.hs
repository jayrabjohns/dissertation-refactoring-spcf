module PCF (Term (..), Type (..), Value (..), Label, Environment, eval, interpret, substitute) where
...
eval (Closure env (Pred term)) = do
  ...
  case val of
    Nat 0 -> Right (Nat 0) -- I think Pred 0 might need to diverge rather than return 0 again
    Nat i -> Right (Nat (i - 1))
